...
@section('container')
    ...
    <div class="flex-col space-y-1">
        <div class="bg-[#FFFFFF] w-screen h-[60px] rounded shadow-md text-[1.7rem] font-bold flex items-center px-5 text-[#56B8FE]">Rooms   
        </div>
        <div class="bg-[#FBFBFB] w-screen min-h-[537px] rounded shadow-md">
            <form action="{{ route('room') }}" method="POST" enctype="multipart/form-data" class="px-30 flex flex-col gap-5">
                @csrf
                <div class="flex flex-col mt-5">
                    <label for="text" class="text-black font-bold text-[1.3rem]">Room Number</label>
                    <input type="text" name="room_number" class="bg-white pl-2 h-10 w-70 border-2 rounded shadow-md" required>
                </div>
                <div class="flex flex-col">
                    <label for="name" class="text-black text-[1.3rem] font-bold">Room Name</label>
                    <input type="text" name="room_name" id="" class="bg-white h-10 pl-2 border-2 w-70 rounded shadow-md" required>
                </div>
                <div class="flex flex-col">
                    ...
                    <div class="relative h-40 w-50 border-2 bg-white rounded shadow-md overflow-hidden">
                        ...
                        <input type="file" name="room_photo" id="image" accept="image/*"
